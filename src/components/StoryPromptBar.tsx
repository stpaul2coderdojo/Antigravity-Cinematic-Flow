...
import { Sparkles, Wand2, Film, Layers, PlayCircle, Palette, BookmarkCheck, ArrowRight, RefreshCw, Radio, CheckCircle2, Award, Zap, Cpu } from 'lucide-react';
...
export const StoryPromptBar: React.FC<StoryPromptBarProps> = ({
  ...
  return (
    <div className="max-w-7xl mx-auto py-6 px-4 space-y-6 animate-fade-in text-[#F0F0F0]">
      {/* Hackathon Judges Callout Banner */}
      {onOpenJudgesGuide && (
        <div className="p-4 rounded-2xl bg-gradient-to-r from-amber-950/50 via-purple-950/40 to-blue-950/50 border border-amber-500/30 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-[0_0_20px_rgba(245,158,11,0.1)]">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="font-bold text-xs text-amber-300 font-mono uppercase tracking-wider">
                  Devpost Hackathon Judges Hub
                </span>
                <span className="text-[9px] px-1.5 py-0.5 rounded bg-amber-500/20 text-amber-200 border border-amber-500/30 font-mono">
                  AGENTIC CINEMA 2026
                </span>
              </div>
              <p className="text-xs text-white/70 mt-0.5">
                Explore the Multi-Agent architecture, rubric alignment, and 1-click test workflows.
              </p>
            </div>
          </div>
